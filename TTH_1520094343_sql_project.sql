...
SELECT DISTINCT Members.firstname AND Members.surname AS Membername, Facilities.name AS Facility
FROM Members
...
WHERE Bookings.starttime = ('2012-09-14') AND
 ((Members.memid = 0 AND Bookings.slots * Facilities.guestcost > 30) OR
  (Members.memid > 0 AND Bookings.slots * Facilities.membercost > 30)) as something
WHERE cost > 30
ORDER BY cost DESC
...
SELECT Facilities.name, SUM(Bookings.slots * (CASE WHEN Bookings.memid = 0 THEN Facilities.guestcost ELSE Facilities.membercost END)) AS revenue
FROM Bookings
INNER JOIN Facilities ON Bookings.facid = Facilities.facid
